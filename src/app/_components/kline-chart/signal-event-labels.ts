...
export function createSignalEventRenderKey(
  ...
  return `${theme}:${activeSignalId ?? "none"}:${candles.length}:${firstCandleTime}:${lastCandleTime}:${signalKey}`;
}
...
export function renderSignalEventLabels({ activeSignal, candles, chart, onSignalSelect, overlay, signals, theme }: SignalEventLabelRenderInput): boolean {
  ...
  for (const cluster of resonanceClusters) {
    overlay.appendChild(createResonanceClusterDecoration(cluster, theme));
  }
...
function createSignalAvatarButton(input: {
  ...
  const { activeSignal, item, onSignalSelect, theme } = input;
  const { signal } = item;
  ...
  button.type = "button";
  button.title = `${signal.source_name} 路 ${signal.created_at.replace("T", " ").slice(0, 16)}`;
...
function createResonanceClusterDecoration(cluster: readonly AvatarLayoutItem[], theme: ChartTheme): HTMLDivElement {
  ...
  tooltip.textContent = `${cluster.length}源共振`;
  tooltip.style.position = "absolute";
  tooltip.style.left = "50%";
