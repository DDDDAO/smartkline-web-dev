...
const MARKET_ALIGNED_SIGNAL_STAGGER_MINUTES = 17;
...
function selectSignalReferenceCandle(candles: readonly MarketCandle[], index: number): MarketCandle | null {
  ...
  const staggerOffset = index * MARKET_ALIGNED_SIGNAL_STAGGER_MINUTES;
  const jitterOffset = (index % 3) * 4;
  const offset = MARKET_ALIGNED_SIGNAL_MINIMUM_OFFSET_MINUTES + (staggerOffset + jitterOffset) % Math.max(1, candles.length - 1);
